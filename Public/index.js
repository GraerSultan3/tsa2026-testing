...
for (let i = 0; i < mainLayout.children[biggerElement].children.length; i++)
{
    ...
    if (child.getBoundingClientRect().bottom + window.scrollY > endHeight + (Number.parseFloat(window.getComputedStyle(document.body).height.substring(0, window.getComputedStyle(document.body).height.length - 2)) * 0.01))
    {
